...
parser.add_argument("--path_data", type=str, help="Path to training data csv.")
parser.add_argument(
    "--path_model",
    type=str,
    help="Path to directory where the trained models" + " are stored.",
)
parser.add_argument("--model_name", type=str, help="Name of the model iteration.")
parser.add_argument("--scaler_feats", type=str, help="Path to the feature scaler.")
...
STRATEGIES = {
    ...
    "jackknife_plus_ab": dict(method="plus", cv=Subsample(n_resamplings=20)),
    "jackknife_minmax_ab": dict(method="minmax", cv=Subsample(n_resamplings=5)),
}

paramsMAPIE = dict(
    **STRATEGIES["jackknife_plus_ab"], n_jobs=-1, agg_function="median", verbose=5
)
model = MapieRegressor(XGBRegressor(**params_), **paramsMAPIE)
...
df_val = pd.DataFrame(
    {"gt": y_val_.ravel(), "pred": pred_.ravel(), "error": errors_.ravel()}
)
...
ax.errorbar(
    data=df_val, x="gt", y="pred", yerr="error", marker="o", linestyle="", alpha=0.4
)
sns.scatterplot(data=df_val, x="gt", y="pred", color="black", ax=ax, s=320)
ax.set_xscale("log")
ax.set_yscale("log")
ax.set_xlim([np.min(y_val_), np.max(y_val_)])
ax.set_ylim([np.min(y_val_), np.max(y_val_)])
(diag_line,) = ax.plot(
    ax.get_xlim(), ax.get_ylim(), ls="--", c="black", linewidth=6, alpha=0.5
)
ax.set(xlabel="Ground Truth (cm$^{-3}$)", ylabel="Model Prediction (cm$^{-3}$)")
...
df_train = pd.DataFrame(
    {"gt": y_.ravel(), "pred": pred_.ravel(), "error": errors_.ravel()}
)
...
fig, ax = plt.subplots(1, 1, figsize=(16, 10))
ax.errorbar(
    data=df_train, x="gt", y="pred", yerr="error", fmt=".", linewidth=3, alpha=0.15
)
sns.scatterplot(data=df_train, color="blue", x="gt", y="pred", ax=ax, s=320)
ax.set_xscale("log")
ax.set_yscale("log")
...
df_res = pd.concat([df_train.assign(data="Training"), df_val.assign(data="Validation")])

sns.set(font_scale=2.5)
sns.set_style("whitegrid")
fig, ax = plt.subplots(1, 1, figsize=(16, 10))
ax.errorbar(
    data=df_res, x="gt", y="pred", yerr="error", fmt=".", linewidth=3, alpha=0.15
)
sns.scatterplot(
    data=df_res,
    color="blue",
...
# ==================================================================== #
df_res.to_parquet(f"{path_results}/{model_prefix}_data.parquet", index=False)
joblib.dump(scaler_target, f"{path_results}/{model_prefix}_target_scaler.joblib")
